...
slower. progressbar2 has three layers aimed at that, each optional and each
falling back cleanly to the layer below it when unavailable. This page
covers what each one does, when it applies, and the measured cost of each,
so you know when reaching for one deliberately is worth it.
...
no ``variables`` kwarg was given, ``unit`` is still the default ``'it'``,
...
Note that this dispatch only happens inside ``progressbar.progressbar()``.
Constructing ``ProgressBar()`` yourself always gets the full class.

Layer 3: the native iterator (opt-in via the ``fast`` extra)
==================================================================

Separately, :py:meth:`ProgressBar.__iter__ <progressbar.bar.ProgressBar.__iter__>`
checks whether the optional ``speedups`` package
...
``update()`` on every single iteration, matching the pre-gate behaviour
...
python benchmarks/report.py``; see that file for the full comparison against
other libraries). Figures are from one measurement environment (CPython
